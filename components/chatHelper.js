...
class chatHelper extends BaseComponent {
  ...
  helperButtonClick(event) {
    ...
    // 对比取消显示
    Object.assign(this.componentData.menuNode.style, { display: "block", top: `${event.clientY - 10}px`, left: `${event.clientX - 10}px` });
  ...
  buildQuickMsgNode() {
    ...
    addNode.innerHTML = `<input style='text-align: left;' type="text" placeholder="别名" class='btn form-control' sct_name="alias" ><input style='text-align: left;' type="text" placeholder="内容" class='btn form-control' sct_name="content"><button sct_name="submit" class='btn form-control'>add</button>`;
    target.appendChild(addNode);
  ...
  buildQuickMsgAllNode() {
    ...
    for (let i = 0; i < this.indexDBData.quickMsgList.length; i++) {
      ...
      htmlText += `<tr><td><button class='btn form-control' sct_index='${i}'>${element.alias}</button></td><td><button class='btn form-control'>删除</button></td></tr>`
    }
  ...
  quickMsgClick(event) {
    ...
    if (index == null) {
      ...
      // 使用快捷消息
      let msg = this.indexDBData.quickMsgList[index].msg;
      this.textareaAdd(msg);
    }
  }
